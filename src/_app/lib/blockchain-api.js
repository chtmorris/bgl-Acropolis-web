...
var web3 = new Web3(new Web3.providers.HttpProvider("http://172.18.4.45:8545"));
...
function contractDeployAssetLoan(fileName, id, fromAccount, accountPass, callback) {
	...
	*/
    ...
    contractObject.new(settings.ASSET_VALUE, settings.FUNDING_TARGET, settings.INTEREST_RATE, settings.MIN_INVESTMENT, settings.PAYMENT_SIZE, // uint _paymentSize,
        ...
        from: owner, data: compiled['data'],gas: *2
    },
    function(e, contract) {callbackContractDeployment(e, contract, id);
...
contractLoad(fileName) {
	...
	*/
    var
    contract, contractData, interface, address; contractData = settings. contracts[fileName];interface = contractData['interface'];address = contractData['address'];if
    (address) contract = web3. eth. contract(interface).at(address);
...
callbackContractDeployment(e, contract, id) {
	...
	*/
    if
    (!e) {if
        (!contract. address) {console. log("Contract transaction sent: TransactionHash: " + contract. transactionHash + " waiting to be mined...");
        } else
...
accountSetupForTransaction(account, password) {
    ...
    if
    (!(account)) {account = settings. FUELING_ACCOUNT; password = settings. DEFAULT_PASSWORD;
    }

    web3. personal. unlockAccount(account, password);return
    account;
}
...
listeners() {var
    ...
    // Event listeners
    contractCreateEventListener(contract, 'InvestmentProposalAddedEvent');contractCreateEventListener(contract, 'InvestmentProposalApprovedEvent');contractCreateEventListener(contract, 'FundingTargetReachedEvent');contractCreateEventListener(contract, 'InvestmentProposalDeclinedEvent');contractCreateEventListener(contract, 'InvestorPaymentMadeEvent');contractCreateEventListener(contract, 'LoanPaidOff');contractCreateEventListener(contract, 'PaymentMadeEvent');
}
